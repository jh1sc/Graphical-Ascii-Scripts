...
function FractalTree {
    ...
    $x2 = $x + [Math]::Cos(($angle + 90) * [Math]::PI / 180) * $length
    $y2 = $y + [Math]::Sin(($angle + 90) * [Math]::PI / 180) * $length
    # draw the current branch
    2dLine $x $y $x2 $y2 "*"
    # recursively draw the left and right branches
    FractalTree $x2 $y2 ($length * 0.8) ($angle + 30) ($depth - 1)
    FractalTree $x2 $y2 ($length * 0.8) ($angle - 30) ($depth - 1)
}
...
while ($true)
{
    ...
    if(ASKS "Q"){$length += 0.5}
    if(ASKS "A"){$length -= 0.5}

    if(ASKS "E"){$angle += 5}
    if(ASKS "D"){$angle -= 5}

    FractalTree 0 0 $length $angle $depth
